...
// prepare insert replicator SQL statement
const prepareInsertReplicatorSQL = (obj) => {
  const fields = ['id', 'source', 'target', 'continuous', 'create_target', 'state', 'seq', 'doc_count']
  const replacements = ['$1', '$2', '$3', '$4', '$5', '$6', '$7', '$8']
  const values = [obj.id, obj.source, obj.target, obj.continuous, obj.create_target, obj.state, obj.seq, obj.doc_count]
  const sql = 'INSERT INTO _replicator (' + fields.join(',') + ') VALUES (' + replacements.join(',') + ')'
  return { sql: sql, values: values }
}
